 ...
 * @LastEditors: Damom
 * @LastEditTime: 2020-12-05 11:36:57
 * @description: 
 -->
<template>
  <div
  ...
  >
    <span class="count-title" v-if="!isProductDetail" :style="color">{{
      title
    }}</span>

    <div
      class="count-item count-number row row-center"
      v-if="count.day1 || count.day0"
    >
      {{ count.day0 }}{{ count.day1 }}
    </div>

    <span
class="count-unit-text count-item "
v-if="count.day1 || count.day0"
      >天</span
    >
    ...
    <div
      class="count-item count-number row row-center"
      :class="[count.hour0 ? '.count-number-long' : '']"
    >
      {{ count.hour0 || '' }}{{ count.hour1 }}{{ count.hour2 }}
    </div>

    <span class="count-item count-unit-text">:</span>

    <div class="count-item count-number row row-center">
      {{ count.minute0 }}{{ count.minute1 }}
    </div>

    <span class="count-item count-unit-text">:</span>

    <div class="count-item count-number row row-center">
      {{ count.second0 }}{{ count.second1 }}
    </div>
